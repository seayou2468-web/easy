...
#elif defined(__APPLE__) && TARGET_OS_IOS
			// iOS app startup is import-first: games are expected in writable Documents.
			// Keep games in a dedicated subdirectory instead of placing project files
			// directly under Documents root.
			auto docs_path = IOSUtils::GetDocumentsDir();
			Output::Info("iOS startup: Documents directory resolved to '{}'", docs_path);
			auto docs_fs = FileFinder::Root().Create(docs_path);
			project_path = FileFinder::MakePath(docs_path, "Games");
			Output::Info("iOS startup: default game directory set to '{}'", project_path);
			if (docs_fs && !docs_fs.Exists("Games")) {
				bool created = docs_fs.MakeDirectory("Games", true);
				Output::Info("iOS startup: creating Games directory result={}", created);
			} else if (!docs_fs) {
				Output::Warning("iOS startup: failed to access Documents filesystem view '{}'", docs_path);
			}
#endif
		}
	}
